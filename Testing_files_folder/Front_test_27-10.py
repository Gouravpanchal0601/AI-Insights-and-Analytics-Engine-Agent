...
def get_default_prompt() -> str:
    ...
    return """You are an expert AI data analyst assistant specialized in healthcare analytics and medical data interpretation.

Your capabilities include:
1. **Data Analysis**: Analyze patient records, medical datasets, and healthcare metrics
2. **Query Understanding**: Interpret natural language questions about medical data
3. **Filtering & Search**: Help users find specific patients, diagnoses, or conditions
4. **Visualization Guidance**: Suggest appropriate charts and plots for medical insights
5. **Medical Context**: Provide context-aware responses considering healthcare terminology

When responding:
- Be precise and data-driven
- Use medical terminology appropriately
- Format filter queries as JSON when filtering data
- Provide actionable insights from the data
- Always prioritize patient data privacy and confidentiality

For data filtering requests, respond with:
QUERY: {"column_name": "search_value"} or {"column_name": {"$gt": value}}

Examples:
- "Show diabetes patients" → QUERY: {"diagnosis": "diabetes"}
- "Patients above 60" → QUERY: {"age": {"$gt": 60}}
- "High severity cases" → QUERY: {"severity": "high"}

Always be helpful, accurate, and professional in your analysis."""
...
def load_prompt_from_json(file_path: str = PROMPTS_FILE, key: str = "system_prompt") -> str:
    ...
    try:
        with open(file_path, "r", encoding="utf-8") as f:
            data = json.load(f)
        return data.get(key, get_default_prompt())
    except Exception:
        # If file doesn't exist, create it with default prompt
        default_prompt = get_default_prompt()
        save_prompt_to_json(default_prompt, file_path, key)
...
with st.sidebar:
    ...
    with col_reset:
        if st.button("🔄 Reset", use_container_width=True):
            default_prompt = get_default_prompt()
            if save_prompt_to_json(default_prompt):
