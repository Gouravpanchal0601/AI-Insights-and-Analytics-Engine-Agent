...
def load_prompt_from_json(file_path: str = "prompts.json", key: str = "system_prompt") -> str:
    ...
    try:
        ...
        st.warning(f"⚠️ Could not load prompt from {file_path}: {e}")
...
if st.button("🚀 Send", type="primary") and user_question:
    ...
    sys_prompt = load_prompt_from_json()  # <-- Load prompt from JSON here
